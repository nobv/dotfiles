...
# their upstreams, and a workflow folder is also where API keys and machine
...
# The self-made workflow's bundle id names a private domain, so it is never
# written down here and never printed — this label stands in for it.
...
# Verifying the bundle id is what catches an HTML error page served instead of
# an archive, or a slug that now points at something else. The move comes last
# so Alfred never scans a half-written folder.
place_workflow() {
...
# Logs nothing: it runs inside a command substitution, where a log line would
# become the result.
catalogue_lookup() {
...
manifest_append() {
    local bundleid="$1" slug="$2"
    # Keep the two columns lined up, unless the id is too long for the gutter
